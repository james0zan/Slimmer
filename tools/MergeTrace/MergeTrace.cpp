...
/// outside enviroment.
///
void MergeTrace(char *inst_file, char *trace_file_name, char *output_file_name,
                set<tuple<uint64_t, uint64_t, int32_t> > &impactful_fun_call) {
  ...
  CompressBuffer dump(output_file_name);
  ...
  while (iter.NextEvent(
      event_label, tid_ptr, id_ptr, 
      addr_ptr, length_ptr, addr2_ptr)) {
    // switch (event_label) {
    ...
    if (event_label == BasicBlockEventLabel) {
      ...
      if (impactful_fun_call.count(make_tuple(
              *tid_ptr, *addr_ptr, FunCount[I(*tid_ptr, *addr_ptr)]))) {
...
int main(int argc, char *argv[]) {
  if (argc != 4 && argc != 5) {
    printf("Usage: merge-trace pin-trace-file inst-file trace-file "
           "[output-file]\n");
    exit(1);
  }
  // A set of function calls that impact the outside enviroment.
  // For each tuple, it is consisting of (thread ID TID, fun address F,
  // execution count C),
  ...
  if (argc == 4) {
    MergeTrace(argv[2], argv[3], 
      "/scratch1/zhangmx/SlimmerMergedTrace",
      ImpactfulFunCall);
  } else {
