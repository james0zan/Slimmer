...
SegmentTree<int> *Merging(vector<SegmentTree<int> *> trees, int new_group) {
  ...
  return new SegmentTree<int>(PARTIAL_SEGMENT, -1, trees[0]->left,
                              trees[0]->right, Merging(l_childs, new_group),
...
///
void ExtractMemoryDependency(char *merged_trace_file_name,
                             char *output_file_name) {
  SegmentTree<DynamicInst> *Addr2LastStore =
      SegmentTree<DynamicInst>::NewTree();
  map<pair<uint64_t, uint32_t>, uint32_t> InstCount;
  map<uint64_t, set<uint32_t> > ArgGroup;

  CompressBuffer output(output_file_name);
  TraceIter iter(merged_trace_file_name);
  SmallestBlock b;
  while (iter.NextSmallestBlock(b)) {
    ...
    if (b.Type == SmallestBlock::MemoryAccessBlock) {
      ...
      DynamicInst dyn_inst =
          DynamicInst(b.TID, ins_id, InstCount[I(b.TID, ins_id)]++);
      ...
      if (Ins[ins_id].Type == InstInfo::StoreInst) {
        ...
        // %lu is depended on:\n",
        //   dyn_inst.Cnt, dyn_inst.ID, Ins[dyn_inst.ID].Code.c_str(),
        // dyn_inst.TID);
        ...
        for (auto j : Addr2LastStore->Collect(b.Addr[0], b.Addr[1])) {
          if (j.type == COVERED_SEGMENT) {
            // printf("\t* the %d-th execution of\n\t  instruction %d, %s\n\t
            // from thread %lu\n",
            //   j.value.Cnt, j.value.ID, Ins[j.value.ID].Code.c_str(),
            // j.value.TID);
...
int main(int argc, char *argv[]) {
  if (argc != 3 && argc != 4) {
    printf("Usage: extract-memory-dependency inst-file merged-trace-file "
           "[output-file]\n");
    exit(1);
  }
