...
public:
  ...
  SegmentTree(SegmentType t, T v, uint64_t l, uint64_t r, SegmentTree<T> *lc,
              SegmentTree<T> *rc)
  ...
  void Set(uint64_t l, uint64_t r, T v) {
    ...
    // If already covered
    if (type == COVERED_SEGMENT && value == v)
      return;
    ...
    if (l < mid) {
      l_child->Set(l, std::min(r, mid), v);
    }
    if (r > mid) {
      r_child->Set(std::max(l, mid), r, v);
    }

    if (l_child != NULL && r_child != NULL &&
        ((l_child->type == COVERED_SEGMENT &&
          r_child->type == COVERED_SEGMENT &&
  ...
  bool Get(uint64_t x, T &v) {
    ...
    if (left % 2 && right % 2)
      mid++;
    if (x < mid) {
      return l_child->Get(x, v);
    } else {
      return r_child->Get(x, v);
    }
  }
  ...
  void Collect2(uint64_t l, uint64_t r, std::vector<Segment<T> > &res) {
    ...
    if (type != PARTIAL_SEGMENT) {
      ...
      if (res.size() >= 2) {
        ...
        if (((second_last.type == EMPTY_SEGMENT &&
              last.type == EMPTY_SEGMENT) ||
             (second_last.type == last.type &&
              second_last.value == last.value)) &&
      ...
      if (left % 2 && right % 2)
        mid++;
      if (l < mid) {
        l_child->Collect2(l, std::min(r, mid), res);
      }
      if (r > mid) {
        r_child->Collect2(std::max(l, mid), r, res);
      }
    }
  }
